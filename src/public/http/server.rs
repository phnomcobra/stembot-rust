...
use crate::core::{
    message::processing::process_message_collection, message::MessageCollection, state::Singleton,
};
        ...
        true => {
            let inbound_message_collection =
                match MessageCollection::try_from(decrypted_request_body) {
                    ...
                    Err(error) => {
                        return Err(format!("failed to parse http response body: {error}").into())
                    }
                };
                ...
                .try_into()
            {
                Ok(bytes) => bytes,
                Err(error) => {
                    return Err(format!("failed to process http response body: {error}").into())
                }
            }
        }
